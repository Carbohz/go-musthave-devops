...
func updateURLValidator(next http.Handler) http.Handler {
...
func metricNameValidator(next http.Handler) http.Handler {
	fn := func(w http.ResponseWriter, r * http.Request) {
		if metricName := chi.URLParam(r, "metricName"); metricName == "" {
			reason := fmt.Sprintf("Invalid url request: empty metric name. Maybe missing /{metric_name}/{metric_value} in the end of url?")
			http.Error(w, reason, http.StatusNotFound)
		}
...
func metricValueValidator(next http.Handler) http.Handler {
	fn := func(w http.ResponseWriter, r * http.Request) {
		if metricValue := chi.URLParam(r, "metricValue"); metricValue == "" {
			reason := fmt.Sprintf("Invalid url request: empty metric value. Maybe missing /{metric_value} in the end of url?")
			http.Error(w, reason, http.StatusNotFound)
		}
